...
-}
block ::
    forall ps s shape ret l rs lbl.
    KnownAppend ps s =>
...
if_ ::
    forall ps s shape ret l rs lbl lv.
    KnownAppend ps s =>
    Expr shape ret l (rs ': lbl) ps rs ->
    Expr shape ret l (rs ': lbl) ps rs ->
...
return_ ::
    forall ret s shape l lbl anyOut.
    KnownAppend ret s =>
