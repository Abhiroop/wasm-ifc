{- | Integer division and remainder (and float min/max/copysign) with WebAssembly's trapping
  and NaN behaviour. Returning @Either Trap@ keeps these independent of the interpreter's
  plumbing. Operands are raw bit patterns; signedness selects the interpretation.
-}
